...
class Employees(Base):
    def open_accordion(self):
        accordion_btn = self.find_element(
            By.CSS_SELECTOR, "button.employees-filter-top__toggle"
        )
        accordion_btn.click()

    def is_accordion_opened(self):
        try:
            employees_filter = self.find_element(
                By.CLASS_NAME, "employees-filter.employees-filter--opened"
            )
    ...
    def check_professions_titles(self):
        ...
        employees_filter_items = self.find_elements(
            By.CSS_SELECTOR, "button.employees-filter-bottom-item__toggle"
        )
        employees_filter_items = employees_filter_items[
            :-1
        ]  # the last element was "show all"

        for profession in employees_filter_items:
            title_before = self.find_element(
                By.CLASS_NAME, "employees-filter-top__title"
            )
            title_before_text = title_before.get_attribute("innerHTML")

            profession.click()

            title_after = self.find_element(
                By.CLASS_NAME, "employees-filter-top__title"
            )
    ...
    def check_existence_of_employees(self):
        ...
        employees_list_panel = self.find_element(
            By.CLASS_NAME, "employees-list-panel-wrapper"
        )
        employees_list_panel = employees_list_panel.find_element(
            By.CLASS_NAME, "employees-list-panel.js-tracking-area"
        )
        employees_filter_items = self.find_elements(
            By.CSS_SELECTOR, "button.employees-filter-bottom-item__toggle"
        )
