...
class NewsNewsUrl(Base):
    ...
    def show_all_news(self):
        ...
        show_more_news_btn = self.find_element(
            By.CLASS_NAME, "btn.filter__btn.js-news-overview-page__show-more"
        )
        news_shown = news_block.find_element(
            By.CLASS_NAME, "js-news-skip"
        ).get_attribute("value")
        news_total = news_block.find_element(
            By.CLASS_NAME, "js-news-total-count"
        ).get_attribute("value")
    ...
    def check_all_news(self, file_name):
        ...
        for item_info in range(len(all_news_info)):
            ...
            if self.is_404_error() is True:
                ...
                news_info_row = (
                    f'"{item_info}","{news_date}","{news_title}","{news_link}"\n'
                )
