...
class ElasticsearchShortResult(Base):
    ...
    def check_pages_elasticsearch(self, file_name: str, search_query: str):
        ...
        flag = True
        autosuggestion_list = self.find_element(
            By.CSS_SELECTOR, "ul.autosuggestion__list"
        )
        autosuggestion_list_results = autosuggestion_list.find_elements(
            By.CSS_SELECTOR, "a.autosuggestion__list-link"
        )
        autosuggestion_list_results = autosuggestion_list_results[
            :-1
        ]  # The last element was the search button.

        for page in autosuggestion_list_results:

            # Get the name of the page.
            page_title = page.find_element(
                By.CLASS_NAME, "autosuggestion__list-title"
            ).text
            ...
            if Base.is_404_error(self) is True:
                flag = False
                self.check_pages_elasticsearch_report(
                    file_name, search_query, page_title, page_url
                )

        return flag

    def check_pages_elasticsearch_report(
        self, file_name, search_query, page_title, page_url
    ):
