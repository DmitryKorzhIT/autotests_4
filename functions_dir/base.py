...
class Base(webdriver.Chrome):
    def __init__(
        self, teardown=False
    ):  # the teardown is a condition for the __exit__ method
        ...
        super(
            Base, self
        ).__init__()  # __init__(options=options) - for running without opening a browser
    ...
    def is_404_error(self):
        try:
            ...
            error_element = self.find_element(
                By.CLASS_NAME, "page-not-found__title._show-tablet"
            )
            error_element = error_element.find_element(
                By.CLASS_NAME, "page-not-found__title-text"
            )
