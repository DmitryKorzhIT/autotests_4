...
class CompetitionsAndResults(Base):
    def competitions_open_accordion_sports(self):
        ...
        accordion_element = self.find_element(
            By.CLASS_NAME, "foldable.foldable--organizations"
        )
        accordion_btn = accordion_element.find_element(
            By.CSS_SELECTOR, "button.foldable-top__toggle"
        )
    ...
    def is_competitions_accordion_sports_opened(self):
        ...
        try:
            self.find_element(
                By.CLASS_NAME, "foldable.foldable--opened.foldable--organizations"
            )
            return True
        except:
            return False
    ...
    def check_sports_titles(self):
        ...
        sports_btns = accordion_element.find_elements(
            By.CLASS_NAME, "organization-filter-bottom-item__toggle"
        )
        sports_btns = sports_btns[:-1]  # the last button is "show all".

        for sport_btn in sports_btns:
            title_before = accordion_element.find_element(
                By.CLASS_NAME, "foldable-top__title"
            )
            title_before_text = title_before.get_attribute("innerHTML")

            self.execute_script("arguments[0].click();", sport_btn)

            title_after = accordion_element.find_element(
                By.CLASS_NAME, "foldable-top__title"
            )
    ...
    def is_competitions_accordion_years_opened(self):
        ...
        try:
            self.find_element(
                By.CLASS_NAME, "foldable.foldable--opened.foldable--years"
            )
            return True
        except:
            return False
    ...
    def check_each_competition(self, file_name, sport_btn_text, year_btn_text):
        """In particular sport category, in particular year, in each month, and in each row
        ...
        try:
            ...
            accordion_elements = self.find_elements(
                By.CSS_SELECTOR, "button.month__toggle"
            )
            ...
            if len(accordion_elements) >= 1:
                for accordion_element in accordion_elements:
                    ...
                    for competition_row in competitions_list:
                        month_name = month_element.find_element(
                            By.CLASS_NAME, "month__name"
                        )
                        month_name_text = month_name.text

                        competition_dates = competition_row.find_element(
                            By.CLASS_NAME, "col.event-item__date"
                        )
                        competition_dates_text = competition_dates.text

                        competition_medal = competition_row.find_element(
                            By.CLASS_NAME, "col.event-item__medal"
                        )
                        competition_medal_text = competition_medal.text

                        competition_type = competition_row.find_element(
                            By.CLASS_NAME, "col.event-item__type"
                        )
                        competition_type_text = competition_type.text

                        competition_type_red = competition_type.find_element(
                            By.CLASS_NAME, "red"
                        )
                        competition_type_red_text = competition_type_red.text

                        competition_athletes = competition_row.find_element(
                            By.CLASS_NAME, "col.event-item__athletes"
                        )
                        ...
                        if (
                            (competition_medal_text != "")
                            and (competition_athletes_text == "")
                        ) or (
                            (competition_medal_text == "")
                            and (competition_athletes_text != "")
                        ):
            ...
            self.competitions_add_to_report(
                file_name=file_name, competition_row_text=competition_row_text
            )
            flag_local = False
            return flag_local
    ...
    def amount_of_sports_titles(self):
        ...
        sports_btns = self.find_elements(
            By.CLASS_NAME, "organization-filter-bottom-item__toggle"
        )
    ...
    def choose_year(self, year_number):
        ...
        accordion_btn = years_block.find_element(
            By.CSS_SELECTOR, "button.foldable-top__toggle"
        )
        self.execute_script("arguments[0].click();", accordion_btn)
